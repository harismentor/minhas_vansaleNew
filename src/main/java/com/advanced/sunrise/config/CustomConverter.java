            ...
            if (number.contains(".")) { // yes
                ...
                if (f != 0.0) {
                    ...
                    switch (theNumber.length()) {
                        case 1:
                            return convertOneDigits(theNumber) + " فاصلة " + convertPointDigitsForArabic(pointValue);
                        case 2:
                            return convertTwoDigits(theNumber) + " فاصلة " + convertPointDigitsForArabic(pointValue);
                        case 3:
                            return convertThreeDigits(theNumber) + " فاصلة " + convertPointDigitsForArabic(pointValue);
                        case 4:
                            return convertFourDigits(theNumber) + " فاصلة " + convertPointDigitsForArabic(pointValue);
                        case 5:
                            return convertFiveDigits(theNumber) + " فاصلة " + convertPointDigitsForArabic(pointValue);
                        case 6:
                            return convertSixDigits(theNumber) + " فاصلة " + convertPointDigitsForArabic(pointValue);
                        default:
                            return "";
                    }
    ...
    private static String convertPointDigitsForArabic(String twoDigits) {
        ...
        try {
            ...
            for (Character c : twoDigits.toCharArray()) {
                ...
                if (c == '0')
                    builder.append("صفر").append(" و ");
                else
                    builder.append(convertOneDigits(String.valueOf(c))).append(" و ");

            }
            String pointNumber = builder.toString();

            if (pointNumber.endsWith(" و ")) {
